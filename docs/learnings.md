...
[2026-05-30] note | Lighthouse 12 + Vercel CDP bug — повторяющийся артефакт. Три аудита периодически фейлят с одной причиной: `Network.getResponseBody: No data found for resource with given identifier`. Эта ошибка валит: (1) `charset` → Best Practices просаживается в 0; (2) `robots-txt` → SEO 92; (3) `llms-txt` → Agentic Browsing 67. Все три файла на CDN живые и отдаются 200 OK (проверено curl). Реальные краулеры/LLM-агенты читают их без проблем. Не пытаться "пофиксить" локально — баг в Lighthouse 12, не в проекте. При следующих Lighthouse-замерах фокус на Performance / Accessibility / CWV — они реальные.

[2026-05-30] feat | public/llms.txt (fe58fa1) — добавлен по спеке llmstxt.org для AI-скраперов (ChatGPT/Perplexity/Claude). H1 + блокквот + секции (Кейсы, Принципы, Стек, Fit, Контакты). 3.5 KB. Lighthouse Agentic метрика не поднимется из-за CDP bug, но реальные AI-агенты файл прочтут — стратегическая ценность для AI-видимости сайта.
